...
import { laboratoryBrandIcon, laboratoryNavItems } from '../../config/laboratoryNav'

const BrandIcon = laboratoryBrandIcon

export function LaboratoryLayout() {
  const dispatch = useAppDispatch()
  const navigate = useNavigate()
  const theme = useAppSelector((s) => s.ui.theme)
  const displayName = useAppSelector((s) => s.auth.displayName) ?? 'Lab staff'
  ...
  return (
    <div className="flex min-h-screen bg-neutral-bg transition-colors duration-200 dark:bg-dark-bg">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-sidebar shrink-0 flex-col bg-lab-sidebar px-4 py-6 text-white transition-transform md:static md:translate-x-0 ${
      ...
      >
        <div className="relative flex items-center justify-between px-1">
          <div className="flex w-full flex-col items-center gap-2">
            <div className="flex h-14 w-14 items-center justify-center rounded-full bg-white shadow-md">
              <BrandIcon className="h-7 w-7 text-lab-main" strokeWidth={1.75} />
            </div>
            <span className="text-xs font-bold tracking-widest text-white/90">Laboratory Portal</span>
          </div>
          <button
            type="button"
        ...
        <nav className="mt-8 flex flex-1 flex-col gap-1 overflow-y-auto text-sm" aria-label="Laboratory">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-xl px-3 py-2.5 font-medium transition-colors ${
                    isActive ? 'bg-lab-highlight text-white' : 'text-white/90 hover:bg-white/10'
                  }`
                }
                onClick={() => setMobileNavOpen(false)}
              >
          ...
          <label className="flex cursor-pointer items-center justify-between gap-3 px-1">
            ...
            <button
              ...
              className={`relative h-7 w-12 shrink-0 rounded-pill transition-colors ${
                darkMode ? 'bg-lab-dark' : 'bg-white/25'
              }`}
            >
              <span
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-10 flex h-topbar shrink-0 items-center gap-3 border-b border-neutral-border/80 bg-white px-4 shadow-sm dark:border-dark-border dark:bg-dark-card md:gap-6 md:px-8">
          ...
          <div className="relative mx-auto hidden min-w-0 max-w-xl flex-1 md:block">
            ...
            <input
              type="search"
              placeholder="Search orders, patients, test IDs…"
              aria-label="Search"
              className="h-11 w-full rounded-pill border-0 bg-neutral-bg py-2 pl-12 pr-5 text-sm text-neutral-text shadow-inner outline-none ring-1 ring-neutral-border/60 placeholder:text-neutral-light focus:ring-2 focus:ring-lab-main/30 dark:bg-dark-bg dark:text-dark-text dark:ring-dark-border"
            />
          </div>
          ...
          <div className="ml-auto flex items-center gap-2 md:gap-4">
            ...
            <button
              type="button"
              className="flex h-10 w-10 items-center justify-center rounded-full bg-lab-surface text-lab-dark transition-colors hover:bg-lab-surface/80 dark:bg-lab-dark/40 dark:text-lab-light"
              aria-label="Settings"
            >
              <Settings className="h-5 w-5" strokeWidth={1.75} />
            </button>
            <div className="hidden items-center gap-3 border-l border-neutral-border pl-4 dark:border-dark-border sm:flex">
              <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-lab-light to-lab-main text-sm font-bold text-white ring-2 ring-lab-surface">
