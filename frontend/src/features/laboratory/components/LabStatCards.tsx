import { AlertTriangle, CheckCircle, Clock, FlaskConical } from 'lucide-react'

type LabStats = {
  pendingTests: number
  inProgress: number
  completedToday: number
  urgentSamples: number
}

const cards = [
  { key: 'pendingTests' as const, label: 'Pending tests', icon: Clock, color: 'from-violet-300 to-lab-main' },
  { key: 'inProgress' as const, label: 'In progress', icon: FlaskConical, color: 'from-purple-300 to-lab-dark' },
  {
    key: 'completedToday' as const,
    label: 'Completed today',
    icon: CheckCircle,
    color: 'from-fuchsia-300 to-lab-main',
  },
  {
    key: 'urgentSamples' as const,
    label: 'Urgent samples',
    icon: AlertTriangle,
    color: 'from-amber-300 to-lab-dark',
  },
]

type LabStatCardsProps = {
  stats: LabStats
}

export function LabStatCards({ stats }: LabStatCardsProps) {
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {cards.map(({ key, label, icon: Icon, color }) => (
        <article
          key={key}
          className="relative overflow-hidden rounded-card border border-neutral-card-border bg-white p-5 shadow-card dark:border-dark-border dark:bg-dark-card"
        >
          <div
            className={`absolute -right-4 -top-4 h-24 w-24 rounded-full bg-gradient-to-br opacity-20 ${color}`}
            aria-hidden
          />
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm text-neutral-muted dark:text-dark-muted">{label}</p>
              <p className="mt-2 text-3xl font-bold text-neutral-text dark:text-dark-text">{stats[key]}</p>
            </div>
            <div className={`flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br text-white ${color}`}>
              <Icon className="h-5 w-5" strokeWidth={1.75} />
            </div>
          </div>
        </article>
      ))}
    </div>
  )
}
